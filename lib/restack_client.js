var restack_client = {
	...
	requestURL:function(type, criteria){
		var api_url = this.getOption('api_url') + '/' + type;
	...
	PUT:function(type, criteria, data, headers, done){
		this.request(
				{uri:this.requestURL(type, criteria),
				 method:'PUT',
				 headers:this.requestHeaders(headers),
	...
	POST:function(type, data, headers, done){
		
		//console.log(this);
		//console.log('doing post');
		
		this.request(
				{uri:this.requestURL(type),
				 method:'POST',
				 headers:this.requestHeaders(headers),
				 json:data
				}, 
	...
	DELETE:function(type, criteria, headers, done){
		this.request(
				{uri:this.requestURL(type, criteria),
				 method:'DELETE',
				 headers:this.requestHeaders(headers)
